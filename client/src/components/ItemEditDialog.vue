<template>
  <n-modal :show="visible" @update:show="val => emit('update:visible', val)" :title="t('stock.editItem')" preset="card" style="max-width: 500px" :mask-closable="false" :segmented="{ footer: true }">
    <div class="edit-form">
      <div class="form-row">
...
watch(() => props.visible, async (val) => {
  if (val && props.item) {
    ...
    form.spec = (props.item as any).spec || '';
    ...
    try {
      const [catRes, locRes] = await Promise.all([
        stockApi.listCategories?.() || Promise.resolve({ data: [] }),
        locationsApi.list(),
      ]);
      const cats = (catRes as any)?.data || catRes || [];
      categoryOptions.value = Array.isArray(cats)
        ? (cats as any[]).map((c: any) => ({ label: c.name, value: c.id }))
        : [];
